...
def convert_tokens(eval_dict, qa_id, y_start_list, y_end_list, no_answer):
    ...
    for qid, y_start, y_end in zip(qa_id, y_start_list, y_end_list):
        ...
        if no_answer and (y_start == 0 or y_end == 0):
            ...
            start_token = y_start
            end_token = y_end
            token2idx = create_token2idx(context)

            orig_text = " ".join(context.split()[token2idx[start_token]:token2idx[end_token + 1]])
            pred_text = tokens2pred(context, spans, start_token, end_token, token2idx)
            final_text = get_final_text(pred_text, orig_text, do_lower_case = True, verbose_logging=False)
            #################################################################################################
            pred_dict[str(qid)] = final_text
            sub_dict[uuid] = final_text
